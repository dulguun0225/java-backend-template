 ...
 * status. Every feature's {@code *ErrorCode} enum implements this, so the render edge, {@link ProblemBody}
 * and the committed catalog snapshot ({@code ErrorCatalogSnapshotTest}) consume the interface and never an
