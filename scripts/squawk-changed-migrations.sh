...
if [[ -n "$base" && "$base" != 0000000000000000000000000000000000000000 ]] && git cat-file -e "$base" 2>/dev/null; then
  mapfile -t files < <(git diff --relative --name-only --diff-filter=AM "$base"...HEAD -- 'src/main/resources/db/migration/*.sql')
else
  mapfile -t files < <(git ls-files -- 'src/main/resources/db/migration/*.sql')
fi
