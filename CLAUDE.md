...
One repo, one service, one micro-frontend. `backend/` is the Java service (API-only). `frontend/` is the
micro-frontend, a separate static deploy, not started yet; its README records the decisions taken and the
gates to wire, and the `frontend` CI job refuses frontend code that has no `check` script. Shared things sit
at the root: `.github/`, `.specify/`, `scripts/`, `docs/`, `compose.yaml`, this file.
...
They exist, they are green, and `mvn verify` in `backend/` is the definition of done.
...
- `cd backend && mvn verify` runs the whole backend wall (Docker required for the integration tests). Nothing is advisory. `scripts/` run from the root.
...
- A new endpoint changes `backend/openapi/v1.json`; the build writes the actual document to `target/` and tells you to review and copy it.
